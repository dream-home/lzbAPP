...
@Component({
  selector: 'page-selectPayer',
  templateUrl: 'selectPayer.html'
})
export class SelectPayerPage{
   ...
   /* showMune(){
    ...
    findPayees(){
        ...
        this.httpService.get('/recharge/findPayees',paramMap).subscribe((respData:any)=>{
  ...
  selectUser(nickName,amount,thisid){    
      //阻止事件冒泡
        ...
        title: '您确定匹配'+nickName+'的'+parseFloat(amount)+'元提现订单吗？',
        message: '',
        buttons: [
          {
            text: '取消',
  ...
  seleuser(selectId){
             var paramMap = {
              ...
              this.httpService.post('/recharge/selectUser',paramMap).subscribe((respData:any)=>{
                  if(respData.code=='0000'){
